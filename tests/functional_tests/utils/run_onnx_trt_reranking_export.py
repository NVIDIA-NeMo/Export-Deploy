...
def get_args():
    ...
    parser.add_argument(
        "--onnx_export_path",
        type=str,
        default="/tmp/onnx_model_reranker/",
        help="Path to store ONNX model.",
    )
    parser.add_argument("--onnx_opset", type=int, default=18, help="ONNX version to use for export.")
    parser.add_argument(
        "--trt_model_path",
        type=str,
...
def export_onnx_trt(args):
    # Base Llama model needs to be adapted to turn it into a reranker model.
    ...
    dynamic_axes_output = {"logits": {0: "batch_size", 1: "logits_dim"}}
