...
def test_reformat_module_names_to_hf():
    ...
    tensors = {
        "q_adapter.linear_in.weight": torch.randn(
            10,
            10,
        ),
        "k_adapter.linear_out.weight": torch.randn(
            10,
            10,
        ),
        "v_adapter.linear_in.weight": torch.randn(
            10,
            10,
        ),
        "lora_dense_attention_adapter.linear_out.weight": torch.randn(
            10,
            10,
        ),
        "lora_4htoh_adapter.linear_in.weight": torch.randn(
            10,
            10,
        ),
        "gate_adapter.linear_out.weight": torch.randn(
            10,
            10,
        ),
        "up_adapter.linear_in.weight": torch.randn(
            10,
            10,
        ),
    }
    ...
    (
        new_tensors,
        module_names,
    ) = reformat_module_names_to_hf(tensors)
...
def test_convert_lora_weights_to_canonical():
    ...
    lora_weights = {
        ...
        "layers.0.lora_hto4h_adapter.linear_in.weight": torch.randn(
            16,
            1024,
        ),
        "layers.0.lora_hto4h_adapter.linear_out.weight": torch.randn(
            2048,
            16,
        ),
    }

    converted_weights = convert_lora_weights_to_canonical(
        config,
        lora_weights,
    )
