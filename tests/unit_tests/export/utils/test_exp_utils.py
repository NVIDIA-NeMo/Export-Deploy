...
class TestUtils:
    ...
    def test_is_nemo2_checkpoint(
        ...
        assert not is_nemo2_checkpoint(
            os.path.join(
                temp_dir,
                "no_context",
            )
    ...
    @pytest.mark.run_only_on("GPU")
    def test_prepare_directory_for_export(
        self,
        temp_dir,
    ):
        from nemo_export.utils.utils import (
    ...
    @pytest.mark.run_only_on("GPU")
    def test_torch_dtype_from_precision(
        self,
    ):
        from nemo_export.utils.utils import (
