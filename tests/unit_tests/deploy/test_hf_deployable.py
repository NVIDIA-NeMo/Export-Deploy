...
class TestHuggingFaceLLMDeploy:
    ...
    def test_initialization_with_model_and_tokenizer(
        self,
    ):
        model = MagicMock(spec=AutoModelForCausalLM)
        tokenizer = MagicMock(spec=AutoTokenizer)
    ...
    def test_initialization_with_model_path(
        self,
        mock_model,
        mock_tokenizer,
    ):
    ...
    def test_triton_input_output_config(
        self,
    ):
        deployer = HuggingFaceLLMDeploy(
            model=MagicMock(),
    ...
    def test_triton_infer_fn_with_error(
        self,
        mock_model,
        mock_tokenizer,
    ):
