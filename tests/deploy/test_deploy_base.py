...
def test_initialization_with_checkpoint():
    with patch('nemo_export_deploy.deploy.deploy_base.ModelPT') as mock_model_pt:
...
def test_is_model_deployable_valid(deploy_base):
    ...
    with patch('nemo_export_deploy.deploy.deploy_base.ITritonDeployable', MockTritonDeployable):
