...
This section demonstrates how to deploy [Megatron-Bridge](https://github.com/NVIDIA-NeMo/Megatron-Bridge) LLM models using Ray Serve. Ray Serve deployment support provides scalable and flexible deployment for Megatron-Bridge LLMs, offering features such as automatic scaling, load balancing, and multi-replica deployment with support for advanced parallelism strategies.
...
2. In a terminal, go to the folder where the ``hf_llama31_8B_mbridge`` checkpoint is located. Pull and run the Docker container image. Replace ``:vr`` with your desired version:
...
3. Deploy the Megatron-Bridge LLM with Ray Serve:
...
3. To use a different model, modify the ``--megatron_checkpoint`` parameter with the path to your Megatron-Bridge checkpoint directory.
...
7. **Checkpoint Loading Issues**: Verify the Megatron-Bridge checkpoint directory path is correct and contains the expected ``iter_*`` subdirectory (e.g. ``iter_0000000``).
