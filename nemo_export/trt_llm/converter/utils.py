...
def save_val(
    ...
    if torch.is_tensor(val):
        ...
        weights_dict[f"{key}{suffix}"].copy_(
            val,
            non_blocking=True,
        )
    else:
...
def get_trt_llm_infix(
    ...
    mapping = [
        ...
        (
            attention_qkv_bias_keys + attention_qkv_weight_keys,
            ".attention.qkv",
        ),
        (
            mlp_router_keys,
    ...
    return sequential_key_map(
        key,
        mapping,
    )


def get_trt_llm_keyname(
    key: str,
) -> str:
    if any_word_in_key(
        key,
...
def split_val_gate(
    ...
    convert_on_device: bool,
):
    if convert_on_device:
        return [
            [n]
            for n in torch.chunk(
                vals[0],
                2,
                axis=-1,
            )
...
def split_and_save_weight(
    ...
    use_attention_nemo_shape = config.get(
    ...
    int8_outputs = config.get(
        "int8_outputs",
        None,
    )
    multi_query_mode = config.get(
        "multi_query_mode",
        False,
    )
    ...
    size_per_head = config.get(
        "kv_channels",
        None,
    )
    convert_on_device = config.get(
        "convert_on_device",
        False,
    )
    is_fp8_model = config.get(
        "fp8_quantized",
        False,
    )
    use_fp8_kv_cache = config.get(
        "fp8_kvcache",
        False,
    )
    ...
    if "layernorm.weight" in key and config.get(
        "apply_layernorm_1p",
        False,
    ):
    ...
    if any_word_in_key(
    ...
    elif any_word_in_key(
        key,
        attention_dense_weight_keys + mlp_proj_weight_keys,
    ):
        if convert_on_device:
            save_val(
                vals[0],
        ...
        if use_fp8_kv_cache:
            ...
            save_val(
                scaling_factor,
                dir,
                base_key + ".kv_cache_scaling_factor",
            )
...
def init_model_parallel_from_nemo(
    reshard_model,
):
