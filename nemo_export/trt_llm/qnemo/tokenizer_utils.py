...
def get_nmt_tokenizer(nemo_checkpoint_path: str):
    ...
    if library == "huggingface":
        LOGGER.info(
            f"Getting HuggingFace AutoTokenizer with pretrained_model_name: {tokenizer_cfg.type}"
        )
        tokenizer = AutoTokenizer.from_pretrained(
            tokenizer_cfg["type"], use_fast=tokenizer_cfg.get("use_fast", False)
        )
    elif library == "sentencepiece":
        LOGGER.info(f"Getting SentencePieceTokenizer with model: {tokenizer_cfg.model}")
        tokenizer = SentencePieceTokenizer(
            model_path=os.path.join(nemo_checkpoint_path, tokenizer_cfg.model),
            legacy=legacy,
        )
