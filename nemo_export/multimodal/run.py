...
class MultimodalModelRunner:
    def __init__(
        ...
        self.num_frames = config["builder_config"].get(
            "num_frames",
            None,
        )
        self.image_size = config["builder_config"].get(
            "image_size",
            None,
        )
    ...
    def preprocess(
        ...
        if self.model_type == "vila":
            ...
            (
                input_ids,
                ptuning_args,
            ) = self.setup_fake_prompts_vila(
                batch_size,
                visual_features,
                first_batch_split_prompts,
                input_lengths,
            )
            return (
                input_ids,
            ...
            input_ids = self.insert_tokens_by_index(
                input_ids,
                num_sample_frames,
            )
    ...
    def tokenizer_image_token(
        ...
        input_ids = input_ids.unsqueeze(0).expand(
            batch_size,
            -1,
        )
    ...
    def setup_fake_prompts(
        ...
        if hasattr(
            self,
            "num_frames",
        ) and (visual_features.shape[1] == self.num_frames):
        ...
        ptuning_args = self.ptuning_setup(
            visual_features,
            input_ids,
            input_lengths,
        )
    ...
    def preprocess_lita_visual(
        ...
        if (
            ...
            num_image_frames = min(
                num_frames,
                config["mm_cfg"]["lita"]["sample_frames"],
            )
            idx = np.round(
                np.linspace(
                    0,
            ...
            im_features = visual_features[
                :,
                idx,
                ...,
            ]

            vid_features = einops.reduce(
                visual_features,
                "b t s d -> b t d",
                "mean",
            )
            ...
            selected_frames = np.round(
                np.linspace(
                    0,
                    visual_features.shape[1] - 1,
                    pool_size * pool_size,
                )
            ).astype(int)
            s_tokens = visual_features[
                :,
                selected_frames,
                ...,
            ]
    ...
    def process_lita_video(
        ...
        if isinstance(
            ...
            num_sample_frames = self.get_num_sample_frames(
                nemo_config,
                vid_len,
            )
            image = (
