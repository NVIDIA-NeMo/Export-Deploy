...
def unpack_tarball(archive: str, dest_dir: str):
    """Unpacks a tarball into a destination directory with path traversal protection.
    ...
    """
    # Ensure destination directory is absolute and normalized
    dest_dir = os.path.abspath(dest_dir)

    with tarfile.open(archive, mode="r") as tar:
        for member in tar.getmembers():
            # Get the member name and normalize it
            ...
            # Normalize the path to prevent any remaining traversal attempts
            normalized_name = os.path.normpath(member_name)

            # Check again after normalization for any remaining traversal
            if normalized_name.startswith("..") or normalized_name.startswith("/"):
                raise ValueError(f"Path traversal attempt detected after normalization: {normalized_name}")
            ...
            if not os.path.abspath(dest_path).startswith(dest_dir):
                raise ValueError(f"Path traversal attempt detected: {member_name} would extract outside destination")
