...
class LlamaConverter(ModelConverter):
    ...
    def convert_weights(
        ...
        yield (
            "model.norm.weight",
            state_dict["model.decoder.final_layernorm.weight"],
        )
        if not nemo_model_config.get(
            "share_embeddings_and_output_weights",
            False,
        ):
            yield (
                "lm_head.weight",
                state_dict["model.output_layer.weight"],
            )
...
class Starcoder2Converter(ModelConverter):
    ...
    def convert_weights(
        ...
        if has_bias:
            yield (
                "model.norm.bias",
                state_dict["model.decoder.final_layernorm.bias"],
            )
        ...
        for layer in range(int(num_layers)):
            ...
            for (
                ...
                qkv_weights_slice = qkv_weights[slice].reshape(
                    -1,
                    hidden_size,
                )
                yield (
